  ...
  return (
    <ModerationShell>
      <header className="space-y-1">
        ...
        <p className="text-sm text-slate-600">Internal queue для trust & safety ops: жалобы, верификации, appeals и proactive risk review.</p>
      </header>
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <OpsStatCard label="Appeals на рассмотрении" value={stats.appealsInReview} />
        <OpsStatCard label="High / urgent очередь" value={stats.urgentOrHigh} />
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <Link href="/admin/moderation/reports" className="inline-flex h-9 items-center rounded-lg bg-slate-900 px-3 text-sm font-semibold text-white">
            Жалобы
          </Link>
          <Link href="/admin/moderation/verification" className="inline-flex h-9 items-center rounded-lg bg-slate-900 px-3 text-sm font-semibold text-white">
            Верификация
          </Link>
          <Link href="/admin/moderation/appeals" className="inline-flex h-9 items-center rounded-lg bg-slate-900 px-3 text-sm font-semibold text-white">
            Appeals
          </Link>
          <Link href="/admin/moderation/enforcement" className="inline-flex h-9 items-center rounded-lg bg-slate-900 px-3 text-sm font-semibold text-white">
            Enforcement
          </Link>
        </div>
        <p className="mt-2 text-xs text-slate-500">Operational note: сначала проверяйте urgent/high priority кейсы.</p>
      </section>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <p className="mt-1 text-sm text-slate-600">Proactive review сигналов из risk layer.</p>
        <ul className="mt-2 space-y-2">
          {highRisk.map((item) => (
            <li key={item.id} className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-sm text-slate-700">
              {item.targetLabel} · {item.priority} · suggested action: review now
            </li>
          ))}
        </ul>
      </section>
