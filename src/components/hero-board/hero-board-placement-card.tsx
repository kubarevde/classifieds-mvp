...
import { HeroBoardPlacement, getStorefrontSellerById } from "@/lib/sellers";
import { getWorldLabel, unifiedCatalogListings } from "@/lib/listings";

type HeroBoardPlacementCardProps = {
  placement: HeroBoardPlacement;
  compact?: boolean;
};
...
export function HeroBoardPlacementCard({ placement, compact = false }: HeroBoardPlacementCardProps) {
  ...
  const listing = placement.listingId
    ? unifiedCatalogListings.find((item) => item.id === placement.listingId) ?? null
    : null;

  const title =
    placement.targetType === "storefront"
      ? seller?.storefrontName ?? "Storefront продавца"
      : listing?.title ?? "Объявление продавца";
  const subtitle =
    placement.targetType === "storefront"
      ? seller?.shortDescription ?? "Выделенное размещение storefront."
      : listing?.price ?? "Выделенное размещение объявления.";
  const destinationHref =
    placement.targetType === "storefront"
      ? `/sellers/${placement.sellerId}`
      : listing?.detailsHref ?? `/listings/${placement.listingId ?? ""}`;
  ...
  return (
    ...
    >
      ...
      <div className={`mt-2 grid gap-3 ${compact ? "md:grid-cols-[minmax(0,1fr)_130px]" : "md:grid-cols-[minmax(0,1fr)_170px]"}`}>
        <div>
          <p className={`${compact ? "text-base" : "text-xl"} font-semibold tracking-tight text-slate-900`}>{title}</p>
          <p className="mt-1 text-sm text-slate-600">{subtitle}</p>
          <p className="mt-2 text-xs text-slate-500">
            Охват: {placement.scope === "global" ? "вся платформа" : getWorldLabel(placement.world ?? "all")} · период:{" "}
