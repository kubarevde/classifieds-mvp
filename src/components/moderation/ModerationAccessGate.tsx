...
export function ModerationAccessGate({ children }: { children: ReactNode }) {
  ...
  const allowed = canAccessModerationConsole({ role });
  if (allowed) return <>{children}</>;

  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 text-center">
      <p className="text-lg font-semibold text-slate-900">Доступ ограничен</p>
      <p className="mt-1 text-sm text-slate-600">
        Moderation console доступна только для внутренней роли moderator/admin (в демо — role all).
      </p>
